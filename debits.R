...
#G0170420 Bresle Ponts-et-Marais
  ...
  sites <- get_hydrometrie_sites(code_departement= str_c(dep))#code utilise pour trouver les stations hydros de mesure de debits

  #### Sorties graphiques et csv ####
  for (case in levels(factor(sites$code_site))){
      
      # Extraction des debits #
      deb1 <- get_hydrometrie_obs_elab(code_entite=case,grandeur_hydro_elab="QmJ",date_debut_obs_elab=as.Date("1900-01-01"),date_fin_obs_elab=as.Date("1950-12-31"))
      deb2 <- get_hydrometrie_obs_elab(code_entite=case,grandeur_hydro_elab="QmJ",date_debut_obs_elab=as.Date("1951-01-01"),date_fin_obs_elab=as.Date("1999-12-31"))
      deb3 <- get_hydrometrie_obs_elab(code_entite=case,grandeur_hydro_elab="QmJ",date_debut_obs_elab=as.Date("2000-01-01"))
      ...
      deb <- deb %>% inner_join(sites,"code_site") %>% 
      ...
      qmj <- deb %>% group_by(falsedate, datayear) %>% summarise(debit= mean(debit)) #debits journaliers pour comparer l'annee en cours avec les annees precedentes
      qmm <- deb %>% group_by(mois, datayear) %>% summarise(debit= mean(debit)) #debits mensuels pour comparer l'annee en cours avec les annees precedentes
      qmm1 <- deb %>% group_by(mois1, datayear) %>% summarise(debit= mean(debit))#debits mensuels pour comparer l'annee en cours avec les annees precedentes pour le graphique journaliers + mensuels par intervalles de temps
      ...
      all_qmj <- deb %>% group_by(falsedate) %>% summarise(debit= mean(debit)) #debits journaliers bruts
      all_qmm <- deb %>% group_by(mois1) %>% summarise(debit= mean(debit)) #debits mensuels bruts
    ...
    # Debits moyens journaliers # 
      plot <- ggplot(qmj, aes(x=falsedate, y=debit, group=datayear, color=datayear, fill=datayear)) +
        ...
        geom_hline(yintercept=mean(qm2$debit), linetype="dashed", color = "darkorange4", linewidth=1) +
        geom_hline(yintercept=mean(qm1$debit), linetype="dashed", color = "dodgerblue4", linewidth=1) +
    ...
    # Debits moyens journaliers et mensuels #
      plot <- ggplot() +
        ...
        geom_hline(yintercept=quantile(all_qmj$debit, probs=(0.75)), linetype="dashed", color="blue3", linewidth=1) +
        geom_hline(yintercept=mean(all_qmj$debit), linetype="solid", color="black", linewidth=1) +
        geom_hline(yintercept=quantile(all_qmj$debit, probs=(0.25)), linetype="dashed", color="green4", linewidth=1) +
        annotate("text", y=max(all_qmj$debit)*1.09, x=as.Date("2000-11-30","%Y-%m-%d"), label=paste("Quantile 75% (QJ75)=", round(quantile(all_qmj$debit, probs=(0.75)),2),"m³/s"),colour="blue3",size=5, fontface="bold") +
        ...
        annotate("text", y=max(all_qmj$debit)*1.01, x=as.Date("2000-11-30","%Y-%m-%d"), label=paste("Quantile 25% (QJ25)=", round(quantile(all_qmj$debit, probs=(0.25)),2),"m³/s"), colour="green4", size=5, fontface="bold") +
...
remove(all_qmj,all_qmm,deb1,deb2,deb3,plot,qmj,qmm,deb,qm1,qm2,qmm1,sites,case,min_annee,dates_mois,dates_mois1,max_year_deb)
